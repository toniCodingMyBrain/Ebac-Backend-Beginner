...
class EstoqueDAOTest {
    ...
    void cadastrarEbuscarPorId() {
        ...
        Fornecedor fornecedorTest = new Fornecedor("Estoque Teste", "12345678990195");
        fornecedor.cadastrar(fornecedorTest);

        //* criando produto
        Produto produtoTest = new Produto("Produto Teste de Estoque", 99.90, fornecedorTest);
        produto.cadastrar(produtoTest);

        //* criando o estoque do produto
        Estoque estoqueProdutoTest = new Estoque(produtoTest, 25);
        estoque.cadastrar(estoqueProdutoTest);

        //* Buscar Estoque
        Estoque estoqueBusca = estoque.buscarPorId(estoqueProdutoTest.getId());

        assertNotNull(estoqueBusca);
        assertEquals(25, estoqueBusca.getQuantidade());
        assertEquals(estoqueProdutoTest.getId(), estoqueBusca.getId());
    }

    @Test
    void atualizarEstoque() {
        Estoque estoqueBusca = estoque.buscarPorProdutoId(25L);
        Estoque novoEstoque = new Estoque(estoqueBusca.getProduto(), 22);
        estoque.alterar(estoqueBusca.getId(), novoEstoque);
        assertEquals(22, novoEstoque.getQuantidade());
    }

    @Test
    void buscarPorId() {
        //* Buscar Estoque
        Estoque estoqueBusca = estoque.buscarPorId(10L);

        assertNotNull(estoqueBusca);
        assertEquals(9, estoqueBusca.getQuantidade());
    }

    @Test
    void buscarTodos() {
        List<Estoque> estoqueList = estoque.buscarTodos();
        assertTrue(estoqueList.toArray().length > 1);
    }

    @Test
    void buscarPorProdutoId() {
        Estoque estoqueBuscaPorProduto = estoque.buscarPorProdutoId(12L);
        assertEquals(3, estoqueBuscaPorProduto.getId());
    }

    @Test
    void excluir() {
        estoque.excluir(20L);
        Estoque estoqueBuscaPorProduto = estoque.buscarPorId(20L);
        assertNull(estoqueBuscaPorProduto);
    }
}